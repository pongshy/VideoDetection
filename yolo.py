...
def yolo_detect_from_video(video_path,
                           frames=10,
                           # label_path='./cfg/coco.names',
                           label_path = 'D:\\university\\GraduationDesign\\try\\SVM\\cfg\\coco.names',
                           # config_path='./cfg/yolov3.cfg',
                           config_path='D:\\university\\GraduationDesign\\try\\SVM\\cfg\\yolov3.cfg',
                           # weights_path='./cfg/yolov3.weights',
                           weights_path='D:\\university\\GraduationDesign\\try\\SVM\\cfg\\yolov3.weights',
                           confidence_thre=0.5,
                           nms_thre=0.3,
                           jpg_quality=80):
    print(video_path)
...
# 读取一个视频，并处理出该视频的物体出现概率，以及视频信息，通常用于接口对外暴露
def yolo_detect_from_video_out(video_path,
                           frames=10,
                           # label_path='./cfg/coco.names',
                           label_path = 'D:\\university\\GraduationDesign\\try\\SVM\\cfg\\coco.names',
                           # config_path='./cfg/yolov3.cfg',
    ...
    video_name = os.path.basename(video_path)
    # video_images = videoprocess.get_one_video_frames(video_path, frames)
    images, info = videoprocess.get_video_images_info(video_path, frames)
    # images = video_images[video_name]
    # print(len(images))
    print(info)
    ...
    video_appear_object_probability = {video_name: objects_probability}
    print(video_appear_object_probability)
    return video_appear_object_probability, info
