...
def tag_with_info(video_path, model_path='./model/sport4.pickle', frames=5):
    # 获取到训练好的classifier
    clsf = train.get_model(model_path)
    images, info = yolo.yolo_detect_from_video_out(video_path, frames)
    data = utils.get_rows(images, False)

    data = np.array(data)
    # svm classifier得出的结果
    pred = clsf.predict(data)
    # flask框架没有numpy.int64，所以需要强制类型转换为python int
    info['tag'] = int(pred[0])
    print('info', info)
    return info
...
if __name__ == '__main__':
    ...
    tag_with_info(video_path, model_path)
